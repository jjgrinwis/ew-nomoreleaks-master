...
export async function responseProvider(request) {
    ...
    const bodyIsValid = hasCredentials && body[UNAME].length > 1 && body[PASSWD].length > 2;
    ...
    if (knownKey && originResponse.ok) {
        return Promise.resolve(createResponse(200, { "Powered-By": ["Akamai EdgeWorkers No More Leaks module"] }, "<html><body><p>Something wrong with your account, Please contact helpdesk.</p></body></html>"));
    }
    if (originResponse.ok) {
        return Promise.resolve(createResponse(originResponse.status, originResponse.getHeaders(), originResponse.body));
    }
    else {
...
async function getKeyFromKeyGenerator(requestBody) {
    try {
        ...
        if (result.ok) {
            ...
            logger.info(`Endpoint generated a key: ${response[KEY].substring(0, 5)}.....`);
            // let's return our generated key
            return response[KEY];
        }
        else {
            logger.error(`Request to ${KEYGENERATOR_URL} failed with status: ${result.status}`);
        }
    }
    catch (error) {
...
async function keyExists(bindvars) {
    // we can't just use:
...
async function originRequest(request, body, knowKey) {
